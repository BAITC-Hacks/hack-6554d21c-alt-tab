...
Локальная конфигурация на 2026-09-23: пользователь добавил `OPENAI_API_KEY` и
`SONIOX_API_KEY` в `.env`; наличие обоих значений проверено без раскрытия секретов.
Действительность ключей и живые STT/LLM/TTS ещё не проверены.
...
Ключи в текущем `.env` уже добавлены. Для живой приёмки остаются перезапуск процессов,
проверка доступа к провайдерам и frontend: микрофон, слышимый ответ, две KK и одна mixed
реплика, ошибки провайдеров и autoplay. Одни unit-тесты этого не подтверждают.
